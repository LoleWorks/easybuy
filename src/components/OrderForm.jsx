...
import { FORMATS, STD_PRICE, OFFER_PRICE, OFFER_QTY, FREE_DELIVERY_LEI, WHATSAPP_NUMBER } from "../data/content.js";
...
const OrderForm = forwardRef(function OrderForm({ t, lang, plan, setPlan }, ref) {
  ...
  async function submitForm(e) {
    e.preventDefault();
    const planLabel = plan === "sub" ? (lang === "ro" ? "abonament" : "подписка") : lang === "ro" ? "comandă unică" : "разовый заказ";
    ...
    try {
      ...
      console.error("submit-order failed, continuing to WhatsApp handoff", err);
    }

    const msg = `Bună, Ana! Sunt ${form.name}. Aș vrea să comand: Bilbor plată ${form.format} x ${form.qty} baxuri (${planLabel}). Localitate: ${form.city}. Adresă: ${form.address}. Telefon: ${form.phone}. Total estimat: ${total} lei.`;
    ...
    <div className="form-wrap" ref={ref}>
      <form className="form-card" onSubmit={submitForm}>
        ...
        <div className="form-fields">
          ...
          <button className="submit-btn" type="submit" disabled={!form.name || !form.phone || !form.address || !form.city}>
            {t.formCta}
          </button>
        </div>
      </form>
